...
class GELU:
    ...
    def forward(self, Z):
        self.Z = Z
        self.A = (Z / 2) * (1 + scipy.special.erf(Z / np.sqrt(2)))
        return self.A
    
    def backward(self, dLdA):
        dLdZ= dLdA * ((1 + scipy.special.erf(self.Z/np.sqrt(2))) / 2 + self.Z / np.sqrt(2*np.pi) * np.exp((-self.Z**2)/2))
        return dLdZ
...
class Softmax:
    ...
    def forward(self, Z):
        ...
        self.A = expZ / np.sum(expZ,axis=1, keepdims=True)  # TODO
