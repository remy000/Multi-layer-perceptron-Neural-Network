...
class BatchNorm1d:
    ...
    def backward(self, dLdBZ):
        ...
        dLdV = -(1/2) * np.sum(dLdNZ * (self.Z-self.M) * np.power(self.V + self.eps, -1.5), axis=0, keepdims=True) # TODO: Compute gradient of loss backprop through variance calculation.
        dNZdM = -(np.power(self.V + self.eps, -0.5)) - ((1/2)*(self.Z-self.M)) * (np.power(self.V+self.eps,-1.5)) * ((-2/self.N) * np.sum(self.Z-self.M, axis=0, keepdims=True))   # TODO: Compute derivative of normalized input with respect to mean.
